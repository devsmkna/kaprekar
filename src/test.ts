import { keprekar } from ".";

/* Assert the number passed to the function return an expected value */
const assertNumber = (num : number, expected : number) : boolean => {
    if (keprekar(num) === expected) {
        console.log("test passed!");
        return true;
    }
    console.log("test failed! Expected ["  + expected + "], got [" + keprekar(num) + "]");
    return false;
}
